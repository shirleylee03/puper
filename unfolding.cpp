...
vector<Binding_unf *> get_bindings(CPN *cpn, CTransition *transition, CUT *cut, UNFPDN *unfpdn, SYNCH *synch) {
    ...
    for (int i = 0; i < T_producer.size(); i++) {
        ...
        if (idx >= synch->q_begin_idx && t_idx < synch->ba_t_begin_idx) {
...
//            bool found = 0;
//            auto E_producer = e.producer;
//            for (auto k = E_producer.begin(); k != E_producer.end(); k++) {
//                if (condition[*k].cpn_index == idx) {
//                    if (cpn->findP_byindex(idx)->gettid() == ms.gettid() ||
//                        (ms.gettid() == productsort && cpn->findP_byindex(idx)->getsid() == ms.getsid())) {
//                        found = 1;
//                        int b_idx = *k;
//                        auto id = "B" + to_string(conditioncount);
//                        auto p_idx = idx;
//                        auto token = condition[b_idx].multiSet.getonlytoken();
//                        add_Condition(id, p_idx, token);
//                        //产生后弧
//                        e.consumer.emplace(conditioncount - 1);
//                        //建立前弧
